...
public class Solution {
    ...
    /**
     * l, r = 0;
     * r追踪到不为0的数开始与前置0交换，如果前置无0，++，如果前置有0，则l指向前置0。交换
     * l
     *
     * @param nums
     */
    ...
    public int[] maxSlidingWindow1(int[] nums, int k) {
        ...
        if (k >= nums.length) {
            return new int[]{max(nums, 0, nums.length)};
        }
    ...
    public String minWindow(String s, String t) {
        ...
        while (r < s.length()) {
            r++;
            if (r < s.length() && tmp.containsKey(s.charAt(r))) {
                smp.put(s.charAt(r), smp.getOrDefault(s.charAt(r), 0) + 1);
            }
            while (check(smp, tmp) && l <= r) {
                if (r - l + 1 < len) {
                    len = r - l + 1;
                    ansL = l;
                    ansR = r + 1;
    ...
    public boolean check(Map<Character, Integer> smp, Map<Character, Integer> tmp) {
        for (Map.Entry<Character, Integer> entry : tmp.entrySet()) {
            if (smp.getOrDefault(entry.getKey(), 0) < entry.getValue()) {
                return false;
            }
        }
        return true;
    ...
    public int[][] merge(int[][] intervals) {
        ...
        for (int i = 1; i < intervals.length; i++) {
            int lR = list.get(list.size() - 1)[1];
            //根据排序可知 cL>=lL, 所以当cL<=lR时, 区间合并，合并后的结果L = lL, R = max(lR, cR)
            int cL = intervals[i][0];
            int cR = intervals[i][1];
            if (cL <= lR) {
                int R = Math.max(lR, cR);
                list.get(list.size() - 1)[1] = R;
            } else {
                list.add(new int[]{cL, cR});
            }
        }
    ...
    public int firstMissingPositive(int[] nums) {
        ...
        for (int i = 1; i < tmp.size(); i++) {
            if (!tmp.get(i).equals(tmp.get(i - 1)) && !tmp.get(i).equals(tmp.get(i - 1) + 1)) {
                return tmp.get(i - 1) + 1;
            }
        }
        return tmp.get(tmp.size() - 1) + 1;
    }

    public int firstMissingPositiveGov(int[] nums) {
        int n = nums.length;
        for (int i = 0; i < n; ++i) {
            if (nums[i] <= 0) {
                nums[i] = n + 1;
    ...
    public int firstMissingPositiveGov2(int[] nums) {
        int n = nums.length;
        for (int i = 0; i < n; ++i) {
    ...
    public void setZeroes(int[][] matrix) {
        Set<Integer> recordZeroRow = new HashSet<>();
        Set<Integer> recordZeroCol = new HashSet<>();
        ...
        for (Integer row : recordZeroRow) {
            matrix[row] = new int[matrix[row].length];
        }

        for (int i = 0; i < matrix.length; i++) {
            for (Integer col : recordZeroCol) {
                matrix[i][col] = 0;
            }
        }
    }
     ...
     * 旋转前后的对找关系, 矩阵为n阶
     * 则 n = matrix.length
     * matrix[row][col] 旋转后位置为 matrix[col][n-row-1]，即：
     * <p>
     * tmp = matrix[col][n-row-1]
     * matrix[row][col] -> matrix[col][n-row-1]
     * tmp1 = matrix[n-row-1][n-col-1]
     * matrix[col][n-row-1] -> matrix[n-row-1][n-col-1]
     * tmp2 = matrix[n-col-1][row]
     ...
     * matrix[col][n-row-1] = 原matrix[row][col] = tmp
     * <p>
     * 另分析不难得知,
     * * 如果n为偶数, 则旋转左上1/4矩阵即可覆盖全部, row=[0...n/2), col=[0...n/2)
     * * 如果n为奇数，则matrix最中间位置不变,则待旋转节点有n*n-1个 ((n*n)-1)/4 = ((n-1)/2) * ((n+1)/2)
     * * * 即 row=[0...(n-1)/2] col=[0...(n+1)/2]
     * * 另 n为偶数的情况下 n/2 = (n+1)/2
     * * *  n为奇数的情况下 (n-1)/2 = n/2
     * * *  综上，无论n为奇数偶数，均取 row=[0...n/2) col=[0...(n+1)/2]
     * <p>
     * *
     */
...
//        int[][] intervals = new int[][]{new int[]{1, 3}, new int[]{2, 6}, new int[]{8, 10}, new int[]{15, 18}, new int[]{1, 9}};
//        System.out.println(Arrays.deepToString(solution.merge(intervals)));
//        System.out.println(Arrays.deepToString(new int[0][2]));
...
//        System.out.println(solution.gcd(35, 10));
...
//        int[] nums = {0, 2, 2, 1, 1};
//        System.out.println(Arrays.toString(nums));
//        System.out.println(solution.firstMissingPositiveGov2(nums));
...
//        System.out.println(Arrays.deepToString(matrix));
        ...
        int[][] matrix = new int[][]{
                new int[]{01, 04, 7, 11, 15, 17},
                new int[]{02, 05, 8, 12, 19, 20},
                new int[]{03, 06, 9, 16, 22, 23},
                new int[]{10, 13, 14, 17, 24, 25},
                new int[]{18, 21, 23, 26, 30, 31},
        };

//        int target = 8;
//        System.out.println(solution.searchMatrix(matrix, target));
//        System.out.println(solution.search(matrix[0], 18));

        final List<Integer> list = Lists.newArrayList(0, 1, 2);
